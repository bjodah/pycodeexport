...
if [[ "${PY_VERSION:0:1}" == "2" ]]; then
    ...
    wget --quiet http://repo.continuum.io/miniconda/Miniconda-3.10.1-Linux-x86_64.sh -O miniconda.sh;
else
    if [[ "${PY_VERSION:0:1}" == "3.4" ]]; then
        echo "PY_VERSION was not set to 2.7 or 3.4"
        exit 1
    fi
    wget --quiet http://repo.continuum.io/miniconda/Miniconda3-3.10.1-Linux-x86_64.sh -O miniconda.sh;
fi
if [[ $? != 0 ]]; then
